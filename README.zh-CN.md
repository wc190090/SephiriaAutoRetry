...
重开开始前，Mod 会先把仍被原版保留的玩家网络对象固定到本层入口。这样重新生成楼层时，地图揭示和 Boss 碰撞检测不会读到死亡房间的旧坐标。
...
离线安装器的备份位于 `文档\Saved Games\SephiriaModBackups\SephiriaAutoRetry`，与游戏存档目录互为同级，安装器会拒绝任何与源目录重叠的备份路径，并忽略旧版遗留的 `Sephiria\ModBackups`。
